...
from preprocess_labels import map_class_vect_to_class
...
model_config = ModelConfig('config/ModelConfig_classtype.json')
...
model.load_weights(test_config.path_model_classtype)

# Load testfile
FILEPATH = 'path_to_testfile' #json raw data
t0 = time.time()
x_test, y_test = load_data_feat_as_np_single(FILEPATH, labeltype='class')
...
y_pred_loc = map_class_vect_to_class(y_pred)
t3 = time.time()
